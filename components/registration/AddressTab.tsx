  ...
  const lookupAddress = async (postalCode: string) => {
    ...
    try {
      ...
      const searchQuery = formattedPostalCode.length === 8 ? 
        `${formattedPostalCode}, Brazil` : formattedPostalCode;
      ...
      if (data.status === "OK" && data.results.length > 0) {
        ...
        let country = "", state = "", city = "", street = "", postalCode = ""
        ...
        if (data.status === "ZERO_RESULTS") {
          ...
          setAddressError("Erro de autenticação da API. Verifique a chave da API.")
        } else {
      ...
      {!isGoogleMapsLoaded && (
        ...
        <div>
          ...
          {errors.number && touchedFields.number && (
            <p className="mt-1 text-sm text-red-500">{errors.number.message as string}</p>
          )}
        </div>
